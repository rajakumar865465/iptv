...
try {
    ...
    const psqlCmd = `ssh -i C:\\Users\\deba_pc.com\\.ssh\\iptv_rsa -o StrictHostKeyChecking=no ubuntu@35.154.128.217 "sudo -u postgres psql -d iptv_db2 -f ~/insert_channels.sql"`;
    const psqlOutput = execSync(psqlCmd, { encoding: 'utf-8', stdio: 'pipe' });
    console.log("Insert complete.");

    console.log("Verifying channel count...");
    const countCmd = `ssh -i C:\\Users\\deba_pc.com\\.ssh\\iptv_rsa -o StrictHostKeyChecking=no ubuntu@35.154.128.217 "sudo -u postgres psql -d iptv_db2 -t -c 'SELECT COUNT(*) FROM channels;'"`;
