...
async function main() {
  const content = fs.readFileSync(__dirname + '/../updated_channels.m3u', 'utf-8');
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith('#EXTINF:')) {
      // e.g. #EXTINF:-1 group-title="Movies",Bollywood HD Russia (576p) [Geo-blocked]
      ...
      // Find channel in DB
      // We will search by exact ILIKE first, then try ILIKE %name%
      ...
      if (result.rows.length > 0) {
        // Just take the first match to avoid updating duplicates
