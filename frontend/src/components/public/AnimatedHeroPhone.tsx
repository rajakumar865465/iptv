...
function HomeScreen() {
  const channels = [
    { n: 'Aaj Tak', l: 'Hindi News', c: 'bg-red-500/10 text-red-400' },
    { n: 'ABP News', l: 'Hindi News', c: 'bg-red-500/10 text-red-400' },
    ...
    <div className="h-full bg-[#080808] flex flex-col text-white">
      ...
      <div className="px-3 pt-3 flex-1 overflow-hidden">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-[7px] text-red-400 font-bold uppercase tracking-[0.12em]">Live Now</span>
        </div>
        <div className="bg-gradient-to-r from-red-600/20 to-transparent rounded-2xl p-3 border border-red-500/15 mb-3">
          <div className="flex items-center gap-3">
            ...
            <div className="w-7 h-7 bg-red-600 rounded-full flex items-center justify-center shrink-0 shadow-lg shadow-red-600/20">
    ...
    <div className="h-full bg-[#080808] flex flex-col text-white">
      ...
      <div className="flex gap-1.5 px-3 py-2 shrink-0 overflow-hidden">
        {['All', 'Hindi', 'Tamil', 'News', 'Movies'].map((cat, i) => (
          <div key={cat} className={`shrink-0 text-[7px] px-2.5 py-0.5 rounded-full font-semibold ${i === 0 ? 'bg-red-600 text-white' : 'bg-white/[0.06] text-slate-500'}`}>
            {cat}
          </div>
        ))}
      </div>
          ...
          {channels.map((ch, i) => (
            <div key={i} className="flex items-center gap-3 bg-white/[0.04] border border-white/[0.04] rounded-xl px-3 py-2.5">
              ...
              <div className="flex items-center gap-1 shrink-0">
                ...
                <span className="text-[6px] text-red-400 font-bold">LIVE</span>
              </div>
            </div>
          ))}
        </div>
    ...
    <div className="h-full bg-black flex flex-col text-white">
      ...
      <div className="mx-2 mt-2 mb-3 rounded-2xl overflow-hidden relative shrink-0" style={{ aspectRatio: '16/9' }}>
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="w-14 h-14 bg-red-600 rounded-full flex items-center justify-center shadow-xl shadow-red-600/30">
            <Play className="w-6 h-6 fill-white text-white ml-1" />
          </div>
        </div>
        <div className="absolute top-3 left-3 flex items-center gap-1.5 z-10">
          <div className="w-2 h-2 bg-red-500 rounded-full animate-pulse" />
          <span className="text-[7px] text-red-400 font-bold bg-black/50 px-1.5 py-0.5 rounded">LIVE</span>
        </div>
      ...
      <div className="px-3 flex-1 overflow-hidden">
        ...
        <div className="flex gap-1.5 mb-3">
          {['Auto', 'HD', '480p', 'Data Saver'].map((q, i) => (
            <div key={q} className={`text-[7px] px-2 py-1 rounded-full font-semibold ${i === 0 ? 'bg-red-600 text-white' : 'bg-white/[0.06] text-slate-500'}`}>
              {q}
            </div>
          ))}
        </div>
    ...
    <div className="h-full bg-[#080808] flex flex-col text-white">
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-5">
        ...
        <div className="w-full bg-white/[0.05] border border-white/[0.08] rounded-xl px-3.5 py-3 mb-3">
          <div className="text-[8px] text-red-400 font-mono tracking-[0.2em]">IPTV-XXXX-XXXX-XXXX</div>
        </div>
        <button className="w-full bg-red-600 rounded-xl py-3 text-center mb-4 text-sm font-bold transition-colors">
          Activate Now
        </button>
  ...
  return (
    <div className="flex flex-col items-center select-none">
      <div className="relative">
        <div className="absolute inset-0 bg-red-600/15 blur-[60px] rounded-full scale-75 translate-y-8 pointer-events-none" />
      ...
      <div className="flex items-center gap-2.5 mt-6">
        {SCREENS.map((s, i) => (
          <button
            ...
            onClick={() => handleDot(i)}
            aria-label={s.label}
            className={`rounded-full transition-all duration-300 cursor-pointer ${i === current ? 'w-7 h-[6px] bg-red-500' : 'w-[6px] h-[6px] bg-white/20 hover:bg-white/40'}`}
          />
        ))}
      </div>
